...
import { calculateEMIBreakdown, getBulletMaturityDate } from "./emiCalculator";
import {
    cancelAllLoanNotifications,
    scheduleEMIReminder,
    scheduleInsuranceReminder,
} from "./notifications";
import {
    getSupabaseConfig,
    getTransactions,
    isUserEmailColumnSupported,
    normalizeMode,
} from "./transactions";
...
export const getInsurances = async () => {
  try {
    ...
    const userEmail =
      (await AsyncStorage.getItem("@gmail_user_email")) || "anonymous";
    ...
    return data.map((ins) => ({
      id: ins.id,
      name:
        ins.name || ins.insuranceName || ins.insurance_name || "Unnamed Policy",
      premiumAmount: parseFloat(
        ins.premiumamount || ins.premium_amount || ins.premiumAmount || 0,
      ),
...
export const saveInsurance = async (insurance) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to save insurance to Supabase: ${response.status} - ${errText}`,
      );
    }
    ...
    return {
      ...
      name: savedIns.name,
      premiumAmount: parseFloat(
        savedIns.premiumamount ||
          savedIns.premium_amount ||
          savedIns.premiumAmount ||
          0,
      ),
      startDate:
        savedIns.startdate || savedIns.start_date || savedIns.startDate,
...
export const deleteInsurance = async (id) => {
  try {
    ...
    if (!response.ok) {
      const errText = await response.text().catch(() => "");
      throw new Error(
        `Failed to delete insurance from Supabase: ${response.status} - ${errText}`,
      );
    }
...
export const updateInsurance = async (id, updates) => {
  try {
    ...
    if (updates.premiumAmount !== undefined)
      patchPayload.premiumamount = parseFloat(updates.premiumAmount);
    if (updates.premium_amount !== undefined)
      patchPayload.premiumamount = parseFloat(updates.premium_amount);
    if (updates.startDate !== undefined)
      patchPayload.startdate = updates.startDate;
    if (updates.start_date !== undefined)
      patchPayload.startdate = updates.start_date;
    if (updates.frequency !== undefined)
      patchPayload.frequency = updates.frequency;
    ...
    if (!response.ok) {
      const errText = await response.text().catch(() => "");
      throw new Error(
        `Failed to update insurance on Supabase: ${response.status} - ${errText}`,
      );
    }
    ...
    return {
      ...
      premiumAmount: parseFloat(
        updatedIns.premiumamount ||
          updatedIns.premium_amount ||
          updatedIns.premiumAmount ||
          0,
      ),
      startDate:
        updatedIns.startdate || updatedIns.start_date || updatedIns.startDate,
      frequency: updatedIns.frequency || "monthly",
      createdAt:
        updatedIns.createdat ||
        updatedIns.created_at ||
        updatedIns.createdAt ||
        new Date().toISOString(),
    };
  } catch (e) {
...
export const getLoans = async () => {
  try {
    ...
    return data.map((loan) => ({
      id: loan.id,
      loanName:
        loan.loanname || loan.loan_name || loan.loanName || "Unnamed Loan",
      ...
      emiAmount: parseFloat(
        loan.emiamount || loan.emi_amount || loan.emiAmount || 0,
      ),
...
export const saveLoan = async (loan) => {
  try {
    ...
    if (!response.ok) {
      const errText = await response.text().catch(() => "");
      throw new Error(
        `Failed to save loan to Supabase: ${response.status} - ${errText}`,
      );
    }
    ...
    return {
      ...
      emiAmount: parseFloat(
        savedLoan.emiamount || savedLoan.emi_amount || savedLoan.emiAmount || 0,
      ),
      startDate:
        savedLoan.startdate || savedLoan.start_date || savedLoan.startDate,
      status: savedLoan.status || "active",
      createdAt:
        savedLoan.createdat ||
        savedLoan.created_at ||
        savedLoan.createdAt ||
        new Date().toISOString(),
      updatedAt:
        savedLoan.updatedat ||
        savedLoan.updated_at ||
        savedLoan.updatedAt ||
        new Date().toISOString(),
    };
  } catch (e) {
...
export const updateLoan = async (loanId, updatedData) => {
  try {
    ...
    const patchPayload = {};
    if (updatedData.loanName !== undefined)
      patchPayload.loanname = updatedData.loanName;
    if (updatedData.loan_name !== undefined)
      patchPayload.loanname = updatedData.loan_name;
    if (updatedData.loanType !== undefined)
      patchPayload.loantype = updatedData.loanType;
    if (updatedData.loan_type !== undefined)
      patchPayload.loantype = updatedData.loan_type;
    if (updatedData.principal !== undefined)
      patchPayload.principal = parseFloat(updatedData.principal);
    if (updatedData.interest !== undefined)
      patchPayload.interest = parseFloat(updatedData.interest);
    if (updatedData.tenure !== undefined)
      patchPayload.tenure = parseInt(updatedData.tenure);
    if (updatedData.emiAmount !== undefined)
      patchPayload.emiamount = parseFloat(updatedData.emiAmount);
    if (updatedData.emi_amount !== undefined)
      patchPayload.emiamount = parseFloat(updatedData.emi_amount);
    if (updatedData.startDate !== undefined)
      patchPayload.startdate = updatedData.startDate;
    if (updatedData.start_date !== undefined)
      patchPayload.startdate = updatedData.start_date;
    ...
    if (!response.ok) {
      const errText = await response.text().catch(() => "");
      throw new Error(
        `Failed to update loan on Supabase: ${response.status} - ${errText}`,
      );
    }

    const data = await response.json();
    const updatedLoan =
      data && data[0] ? data[0] : { id: loanId, ...updatedData };
    ...
    return {
      ...
      loanName:
        ...
        updatedLoan.loan_name ||
        updatedLoan.loanName ||
        "Unnamed Loan",
      loanType:
        updatedLoan.loantype ||
        updatedLoan.loan_type ||
        updatedLoan.loanType ||
        "emi",
      ...
      emiAmount: parseFloat(
        updatedLoan.emiamount ||
          updatedLoan.emi_amount ||
          updatedLoan.emiAmount ||
          0,
      ),
      startDate:
...
export const deleteLoan = async (loanId) => {
  try {
    ...
    if (!response.ok) {
      const errText = await response.text().catch(() => "");
      throw new Error(
        `Failed to delete loan from Supabase: ${response.status} - ${errText}`,
      );
    }
...
export const addPayment = async (payment) => {
  try {
    ...
    if (!response.ok) {
      const errText = await response.text().catch(() => "");
      throw new Error(
        `Failed to add payment on Supabase: ${response.status} - ${errText}`,
      );
    }
    ...
    return {
      id: savedPayment.id,
      loanId:
        savedPayment.loanid || savedPayment.loan_id || savedPayment.loanId,
      amount: parseFloat(savedPayment.amount || 0),
      paidAt:
        savedPayment.paidat || savedPayment.paid_at || savedPayment.paidAt,
      createdAt:
...
export const calculateLoanStats = (loans, payments = [], insurances = []) => {
  ...
  loans.forEach((loan) => {
    ...
    if (loan.startDate) {
      ...
      monthsElapsed =
        (currentTime.getFullYear() - startDate.getFullYear()) * 12 +
    ...
    const loanInterestSaved = Math.max(
      0,
      standardInterest - breakdown.totalInterest,
    );
      ...
      if (loanType === "emi") {
        ...
        if (monthsFromStart <= tenure) {
          ...
          if (!nextDueDate || nextDue < nextDueDate) {
            ...
            nextDueDate &&
            nextDue.getTime() === nextDueDate.getTime()
          ) {
            nextPaymentAmount += emiAmount;
          }
        }
        ...
        const emiDueThisMonth = new Date(
          currentYear,
          currentMonth,
          startDate.getDate(),
        );
        ...
        const nextDue = getBulletMaturityDate(loan.startDate, tenure);
        const bulletAmount =
...
export const importAllData = async (jsonString) => {
  try {
    ...
    await fetch(`${cleanUrl}/rest/v1/payments${deleteFilter}`, {
      method: "DELETE",
      headers: deleteHeaders,
    });
    await fetch(`${cleanUrl}/rest/v1/loans${deleteFilter}`, {
      method: "DELETE",
      headers: deleteHeaders,
    });
    await fetch(`${cleanUrl}/rest/v1/insurances${deleteFilter}`, {
      method: "DELETE",
      headers: deleteHeaders,
    });
    await fetch(`${cleanUrl}/rest/v1/transactions${deleteFilter}`, {
      method: "DELETE",
      headers: deleteHeaders,
    });
    ...
    const importedInsurances = (data.insurances || []).map((i) => {
      const insObj = {
        ...
        premiumamount: cleanNumeric(
          i.premiumAmount || i.premium_amount || i.premiumamount,
        ),
        startdate:
          i.startDate ||
          i.start_date ||
    ...
    const cleanPayments = importedPayments.filter((p) =>
      validLoanIds.has(p.loanid),
    );
    ...
    if (importedTxs.length > 0) {
      ...
      if (!res.ok) {
        const txt = await res.text().catch(() => "");
        throw new Error(
          `Failed to upload transactions: ${res.status} - ${txt}`,
        );
      }
    }
  ...
  try {
    ...
    if (url && key) {
      ...
      await fetch(`${cleanUrl}/rest/v1/user_settings`, {
        ...
        console.warn("Supabase plan_settings sync skipped/failed:", err),
      );
    }
  } catch (e) {
